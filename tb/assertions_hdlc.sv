...
module assertions_hdlc (
  ...
  // RX flag detection: flag should be reported two cycles after pattern.
  property p_rx_flag_detect;
    @(posedge Clk)
      seq_rx_flag |-> ##2 Rx_FlagDetect;
  endproperty

  // Start flag appears 2 cycles after Tx_ValidFrame rises:
  ...
  // End flag appears 1 cycle after Tx_ValidFrame falls:
  ...
  // Non-byte-aligned or FCS-error condition must raise Rx_FrameError.
  property p_rx_frame_error_on_error_cause;
    @(posedge Clk) disable iff (!Rst)
      seq_rx_frame_error_cause |-> ##[1:2] Rx_FrameError;
  endproperty

  // Normal TX completion should assert Tx_Done.
  property p_tx_done_on_tx_complete;
    @(posedge Clk) disable iff (!Rst)
      seq_tx_frame_end_non_abort |-> Tx_Done;
  endproperty

  // After writing 126 or more bytes, Tx_Full should assert.
  ...
  // Spec 6b: RX inserted-zero detection behavior.
  ...
  // Spec 6c: Inserted-zero cycles must not produce Rx_NewByte.
  ...
  // Spec 10: Rx_AbortSignal after abort detect.
  ...
  // Spec 16: Frame error assertion for framing/FCS faults.
